...
public abstract class ModelLoader implements gl.Renderable {
	...
	public ModelLoader(GLRenderer renderer, String fileName, String textureFileName) {
	...
	private void loadModelFromFile() {
		...
		Log.e("loadModelFromFile", "texture= " + textureFileName + ", model= " + fileName);

		try {
			if (textureFileName != null)
				texture = new Texture(Gdx.files.internal(textureFileName), false);
		} catch (Exception e) {
		...
		if (fileName.endsWith(".dae")){
			...
			Log.e("ColladaLoader", "filename= " + fileName);
		}
			...
			keyFramedModel = new MD2Loader().load(Gdx.files.internal(fileName), 1 / 7f);
			if (texture != null) {
				Material material = new Material("materialTODO", new TextureAttribute(texture, 0, "a_tex0")); //TODO
			...
			FileHandle skeletonFile = Gdx.files.internal(fileName.replace("mesh.xml", "skeleton.xml"));
			skeletonModel = new OgreXmlLoader().load(Gdx.files.internal(fileName), skeletonFile);
			if (texture != null) {
	...
	@Override
	public void render(javax.microedition.khronos.opengles.GL10 gl, gl.Renderable parent, ParentStack stack) {
